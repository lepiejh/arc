 ...
 */
package com.brick.shield.app.camera;
...
import com.brick.shield.util.SystemBar;
...
import com.brick.shield.util.Uri2PathUtil;
...
public class CameraActivity extends BaseActivity {
    ...
    protected void onPermissionGranted(int code) {
        mUri = AlbumUtils.getUri(this, new File(mCameraFilePath));
        switch (code) {
            case CODE_PERMISSION_IMAGE: {
                AlbumUtils.takeImage(this, CODE_ACTIVITY_TAKE_IMAGE, mUri);
                break;
            }
            case CODE_PERMISSION_VIDEO: {
                AlbumUtils.takeVideo(this, CODE_ACTIVITY_TAKE_VIDEO, mUri, mQuality, mLimitDuration, mLimitBytes);
                break;
            }
            default: {
                throw new AssertionError("This should not be the case.");
    ...
    private void callbackResult() {
        if (sResult != null) {
            if (Build.VERSION.SDK_INT >= 29){
                sResult.onAction(Uri2PathUtil.getRealPathFromUri(CameraActivity.this, mUri));
            }else {
